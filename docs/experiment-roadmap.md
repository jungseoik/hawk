...
| 데이터셋 | 보정 전 | 보정 후 | 변화 | 정렬 실패 |
|---|---|---|---|---|
| DoTA | 0.383 | **0.398** | +0.015 | 0/320 |
| UCF-Crime | 0.078 | 0.092 | +0.014 | 8/320 |
| UBnormal | 0.042 | 0.043 | +0.001 | 0/320 |
| ShanghaiTech | 0.094 | 0.111 | +0.017 | 0/320 |

**정렬은 전부 성공했는데 마스크가 줄지 않았다.** 이유는 근본적이다 — 대시캠의 전진
운동은 **깊이에 따라 플로우가 달라지는 3D 시차**를 만든다. 가까운 노면은 빠르게, 먼
건물은 느리게 흐르므로 **어떤 2D 변환도 두 쪽을 동시에 정지시킬 수 없다.** RANSAC 이
inlier 를 찾은 것은 배경 대응의 다수를 맞췄다는 뜻일 뿐이다.

즉 DoTA 의 높은 마스크 비율은 제거 가능한 카메라 운동이 아니라 **실제 시차**에서 온다.
해결에는 깊이 추정이 필요하며 이는 별개의 연구다.
