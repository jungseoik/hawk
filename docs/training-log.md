...
## 2026-08-28 — `L_BL` 1차 실행 **폐기**, fp16 오버플로 수정 후 재실행

### 무슨 일이 있었나

`L_BL` 복원 실험(`bl_flow`·`bl_zero`, 12 epoch, 22시간)이 **완주했지만 결과가 전부 쓰레기**다.
최종 체크포인트의 파라미터 **193/231 이 비유한**(v1 `duplicate` 발산과 같은 서명).

원인은 내가 추가한 `L_BL` forward 였다. iteration 0 부터 `backgroundloss` 만 NaN 이었고
(`oriloss` 는 정상 1.75), 약 300 iteration 만에 그 기울기가 모델 전체를 오염시켰다.
epoch 평균은 12/12 전부 `nan`, iteration 단위로는 3,012 개 중 2,998 개가 NaN 이었다.

### 원인 — fp16 오버플로

같은 입력·같은 체크포인트에서 정밀도만 바꿔 재현:

| 정밀도 | `L_BL` |
|---|---|
| fp16 (`maybe_autocast`) | **nan** |
| bf16 | 4.2606 |
| fp32 | 4.25 |

메인 forward 는 시퀀스가 길고 시각 토큰이 텍스트에 둘러싸여 있어 견디지만, `L_BL` 은
`bos + 시각 32 토큰`으로 시작해 조건이 다르다. 확률적으로 터져서(전부는 아님 — 그래서
드물게 finite 값이 찍혔고, **나는 그 값을 보고 "손실이 하강 중"이라고 잘못 보고했다**)
초반에 알아채지 못했다.

### 수정

1. `L_BL` forward 만 **bf16 autocast** 로 돌린다. 지수 범위가 fp32 와 같아 속도·메모리
   손해 없이 해결된다.
2. 그래도 비유한이면 그 스텝의 배경 감독을 버린다(`loss_background = loss`) — 한 번의
   NaN 이 학습 전체를 죽이지 못하게 하는 안전장치.

스모크 확인: iteration 17 개 중 NaN **0 개**, `backgroundloss` 2.58~2.98 정상 범위.

### 재발 방지 — 앞으로 장시간 학습은 첫 200 iteration 의 NaN 비율을 먼저 본다

epoch 평균만 보면 안 된다. 이미 알려진 함정("1 개 NaN 이 평균을 오염시킨다")과
**진짜 발산**이 로그상 똑같이 `nan` 으로 보이기 때문이다. 구분 기준은 iteration 단위
NaN 비율이다 — 이번은 99.5% 였다.
