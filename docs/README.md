...
| [`training_plan.md`](training_plan.md) | **학습 전체 계획** — 끊어학습(chunked resume) 워크플로, Phase 0(코어)→1(Stage2)→2(ablation)→3(진단), 논문 C1–C5/E1–E4 매핑 |
| [`MIGRATION.md`](MIGRATION.md) | **⭐ 서버 이관 & resume 가이드** — 자산 인벤토리(git/HF/재다운로드), 새 서버 부트스트랩(clone→env→weights→data→HF체크포인트→resume), 경로/토큰 조정 |
